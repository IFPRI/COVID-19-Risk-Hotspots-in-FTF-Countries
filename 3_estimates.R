...
indir <- file.path(dir, country, "population")
rr <- list.files(indir, pattern = ".tif$", full.names = TRUE)

# createHRScore <- function(r, risktable){
#   key <- basename(r)
# }
v <- getData("GADM", country = "BGD", level = 2, path = indir)

bmi <- read.csv("data/DHS_bmi_cluster_level.csv", stringsAsFactors = FALSE)
bmi <- bmi[bmi$country_code == "BD",]
bmis <- bmi[complete.cases(bmi[,c("latnum", "longnum", "wom_bmi")]), c("latnum", "longnum", "wom_bmi")]
...
vmean <- aggregate(bmis, v, mean, na.rm = TRUE)
vmax <- aggregate(bmis, v, max, na.rm = TRUE)
vmin <- aggregate(bmis, v, min, na.rm = TRUE)
...
png(file = "data/bmi_max.png", height=6, width=6, units = "in", res=300)

spplot(v,"maxrisk", col.regions=cols, main="max_covid_risk_bmi", 
